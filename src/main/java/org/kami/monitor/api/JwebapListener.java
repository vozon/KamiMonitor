...
public class JwebapListener implements ServletContextListener {
	...
	public void contextInitialized(ServletContextEvent contextEnvent) {
		...
		try {
			Assert.assertNotNull(configPath,
					"please make sure your application cantains context-param '"
							+ CONFIG_PARAM_NAME + "' in web.xml .");
			...
			if (path != null) {
				...
				Assert.assertNotNull(url,
						"jwebap config not found : context-param '"
								+ CONFIG_PARAM_NAME + "'= " + configPath);
				Startup.startup(url);
			}
		} catch (Throwable e) {
